...
export const ShootingStars = ({
  ...
  useEffect(() => {
    const getRandomStartPoint = () => {
      const width = svgRef.current?.clientWidth || (typeof window !== "undefined" ? window.innerWidth : 800);
      const height = svgRef.current?.clientHeight || (typeof window !== "undefined" ? window.innerHeight : 600);
  ...
  return (
    <svg ref={svgRef} className={cn("w-full h-full absolute inset-0 overflow-hidden pointer-events-none", className)}>
      {star && (
        <rect
          key={star.id}
          x={star.x}
